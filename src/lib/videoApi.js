  ...
  switch (family) {
    ...
    case 'seedance':
      if (settings.model === 'seedance2.0-933') {
        return createSeedance933Task({ token, prompt, settings, referenceImages, seedanceInputs: nextSeedanceInputs });
      }
      return createSeedanceManxueTask({ token, prompt, settings, referenceImages, seedanceInputs: nextSeedanceInputs });
    case 'seedance25':
      return createSeedance25Task({ token, prompt, settings, referenceImages, seedanceInputs: nextSeedanceInputs });
    case 'seedance25gz':
